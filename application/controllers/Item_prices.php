...
class Item_prices extends Secure_Controller
{
	...
	public function view($item_id = NEW_ITEM)
	{
		...
		$data['item_tax_info'] = $this->xss_clean($this->Item_price_taxes->get_info($item_id));
		...
		foreach($stock_locations as $location)
		{
			...
			$quantity = $this->xss_clean($this->Item_price_quantity->get_item_quantity($item_id, $location['location_id'])->quantity);
		...
		{
			$low_sell_item_info = $this->Item_price->get_info($item_info->low_sell_item_id);
	...
	public function generate_barcodes($item_ids)
	{
		...
		$result = $this->Item_price->get_multiple_info($item_ids, $this->item_lib->get_item_location())->result_array();
		...
		foreach($result as &$item)
		{
			...
			if(empty($item['item_number']) && $this->config->item('barcode_generate_if_empty'))
			{
				...
				$this->Item_price->save($save_item, $item['item_id']);
			}
		}
		$data['item_prices'] = $result;
	...
	public function save($item_id = NEW_ITEM)
	{
		...
		if($this->Item_price->save($item_data, $item_id))
		{
			...
			if(!$use_destination_based_tax)
			{
				...
				$success &= $this->Item_price_taxes->save($items_taxes_data, $item_id);
			}
			...
			foreach($stock_locations as $location)
			{
				...
				$item_quantity = $this->Item_price_quantity->get_item_quantity($item_id, $location['location_id']);

				if($item_quantity->quantity != $updated_quantity || $new_item)
				{
					$success &= $this->Item_price_quantity->save($location_detail, $item_id, $location['location_id']);
	...
	{
		$exists = $this->Item_price->item_number_exists($this->input->post('item_number'), $this->input->post('item_id'));
	...
	public function check_kit_exists()
	{
		...
		{
			$exists = $this->Item_price_kit->item_kit_exists_for_name($this->input->post('name'));
		}
		else
		{
			$exists = FALSE;
	...
	public function remove_logo($item_id)
	{
		...
		$result = $this->Item_price->save($item_data, $item_id);
	...
	{
		$employee_id = $this->Employee->get_logged_in_employee_info()->person_id;
		$cur_item_info = $this->Item_price->get_info($item_id);
	...
	//Update stock quantity
		$item_quantity = $this->Item_price_quantity->get_item_quantity($item_id, $location_id);
		...
		if($this->Item_price_quantity->save($item_quantity_data, $item_id, $location_id))
		{
	...
	public function bulk_update()
	{
		...
		if(empty($item_data) || $this->Item_price->update_multiple($item_data, $items_to_update))
		{
			$items_taxes_data = [];
			$tax_names = $this->input->post('tax_names');
			$tax_percents = $this->input->post('tax_percents');
			...
			if($tax_updated)
			{
				$this->Item_price_taxes->save_multiple($items_taxes_data, $items_to_update);
			}
